...
class ProtPySegImportSubtomos(Protocol, ProtTomoBase):
    ...
    def importSubtomogramsStep(self):
        ...
        starf = self._getExtraPath('pyseg.star')
        copyFile(self.starFile.get(), starf)
        self.fhTable = open(starf, 'r')
        for i in range(25):  # Read lines until the first line with data
            nline = next(self.fhTable)

        for line in self.fhTable:  # TODO: does it work??
            readStarfileRow(self, subtomo)
            fileName = subtomo.getFileName()
            # fileName = '/home/estrellafg/scipion3/data/tests/tomo-em/particles/import_particle_000003.mrc'
